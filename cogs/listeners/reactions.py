...
class ReactionsCog(commands.Cog):
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        if log_channel_id == None and payload.emoji.name == '🚫':
            ...
            if payload.member.guild_permissions.administrator == True or payload.member.guild_permissions.manage_messages == True or payload.user_id == reacted_message.author.id:
                await reacted_message.delete()
                description = "*The message has been deleted as it has been reported by an admin.*"

            if not payload.user_id == reacted_message.author.id and get(reacted_message.reactions, emoji=payload.emoji.name).count == 1:
                    ...
                    for role in reacted_message.author.roles:
                        if role.name != '@everyone':
                            if reacted_message_author_roles is None:
                                ...
                                reacted_message_author_roles = str(reacted_message_author_roles) + ", " + str(role.mention)
                ...
                if reacted_message.author.avatar == None:
                    reported_message_embed.set_thumbnail(url="https://cdn.discordapp.com/embed/avatars/0.png")
                else:
                    reported_message_embed.set_thumbnail(url=reacted_message.author.avatar.url)
                reported_message_embed.add_field(name="Username", value=reacted_message.author.name, inline=True)
                if is_member == True:
                    reported_message_embed.add_field(name="Nickname", value=reacted_message.author.nick, inline=True)
                reported_message_embed.add_field(name="User ID", value="<@" + str(reacted_message.author.id) + ">", inline=True)
                ...
                reported_message_embed.add_field(name="Message creation date/time", value="<t:" + str(message_creation_timestamp) + ":D> - <t:" + str(message_creation_timestamp) + ":T>", inline=True)
                #if the message is too long
                if len(reacted_message.content) >= 1000:
                    long_message = True
                    message_length = len(reacted_message.content)
                    reacted_message_content_part2 = reacted_message.content[999:message_length]
                    reacted_message_content_part1 = reacted_message.content[0:999]
                    reported_message_embed.add_field(name="Message", value=reacted_message_content_part1, inline=False)
                    ...
                    reported_message_part2_embed.set_footer(text="Reported by " + str(payload.member.name) + "#" + str(payload.member.discriminator) + "  •  " + now, icon_url=payload.member.avatar.url)
                else:
                    #if the message only contains an attachment
                    if len(reacted_message.content) != 0:
                        reported_message_embed.add_field(name="Message", value=reacted_message.content, inline=False)
                    else:
                    ...
                    reported_message_embed.set_footer(text="Reported by " + str(payload.member.name) + "#" + str(payload.member.discriminator) + "  •  " + now, icon_url=payload.member.avatar.url)
