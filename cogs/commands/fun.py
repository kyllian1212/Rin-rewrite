...
class FunCog(commands.Cog):
    ...
    async def roll(self, interaction: discord.Interaction, number_of_dices: int = 1, number_of_sides: int = 6):
        try:
            ...
            if not 0 < number_of_dices < 101 or not 0 < number_of_sides < 101:
                await interaction.followup.send(content="You cannot roll less than 1 dices and more than 100 dices, and the dices cannot have less than 1 side and more than 100 sides!", ephemeral=True)
            else:
                number_of_rolls = 0
                result = ""
                total = 0
                ...
                if number_of_dices == 1:
                    await interaction.followup.send("**" + str(number_of_dices) + "d" + str(number_of_sides) + " roll result: **" + str(result))
                else:
                    await interaction.followup.send("**" + str(number_of_dices) + "d" + str(number_of_sides) + " roll result: **" + str(result) + "\n**Total: **" + str(total))
        except:
            await embeds.error_executing_command(interaction)
    ...
    async def october18(self, interaction: discord.Interaction):
        try:
            interaction.response.defer()
            await interaction.followup.send(content="https://media.discordapp.net/attachments/186610204023062528/1036707347881525360/IMG_4366.jpg")
        except:
            await embeds.error_executing_command(interaction)
            raise
