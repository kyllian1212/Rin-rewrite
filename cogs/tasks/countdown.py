...
class SmileCountdownCog(commands.Cog):
    ...
    async def days_to_smile_auto(self):
        ...
        if diffs_float <= 1:
            ...
            smile_message_embed.add_field(name="SMILE! :D is out now in more countries!", value="(you can VPN to listen or wait until it's out in your country at midnight local time. do not pirate or send files here or you will get banned)", inline=False)
        elif diffm <= 1:
            smile_message_embed = discord.Embed(title="There are " + str(diffs) + " seconds left before SMILE! :D is released", description="(in New Zealand)", color=0x00aeff)
        elif diffh <= 2:
            smile_message_embed = discord.Embed(title="There are " + str(diffm) + " minutes (" + str(diffs) + " seconds) left before SMILE! :D is released", description="(in New Zealand)", color=0x00aeff)
        elif diffh < 100:
            smile_message_embed = discord.Embed(title="There are " + str(diffh) + " hours (" + str(diffm) + " minutes, " + str(diffs) + " seconds) left before SMILE! :D is released", description="(in New Zealand)", color=0x00aeff)
        elif diffh >= 100:
            smile_message_embed = discord.Embed(title="There are " + str(diffd) + " days (" + str(diffh) + " hours, " + str(diffm) + " minutes, " + str(diffs) + " seconds) left before SMILE! :D is released", description="(in New Zealand)", color=0x00aeff)
